            ...
            <div key={footerLinks.key} className='flex flex-col ss:my-0 my-4 min-w-[150px]'>
              ...
              <ul className='list-none mt-4'>
                {footerlink.links.map((link , index) => (
                  <li key={link.name} className={`font-poppins font-normal text-[16px] leading-[24px] text-dimWhite hover:text-secondary cursor-pointer ${index !== link.length - 1 ? 'mb-4' : 'mb-0'}`}>
                {link.name}
                  </li>
                ))}
              </ul>
      ...
      <div className='flex w-full justify-between items-center md:flex-row flex-col mt-6 border-t-[1px] border-t-[#3F3E45]'>
          ...
          <div className='flex flex-row md:mt-0 mt-6'>
            {socialMedia.map((socialMedia, index) => (
              <img key={socialMedia.id} src={socialMedia.icon} alt={socialMedia.id} className={`w-[21px] h-[21px] object-contain cursor-pointer ${index !==socialMedia.length - 1 ? 'mr-6' : 'mr-0'}`}/>
            ))}
          </div>
      </div>
    </section>
